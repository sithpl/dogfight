...
func _ready():
	...
	if hud:
		hud._set_score(score)  # show current score (defaults to 0 at start)
	...
	# Transmission test
	# ROB64 welcome
	_schedule_transmission(3.0, 1)
	# ROB64 basics
	_schedule_transmission(5.0, 2)
	# Slippy hold A
	_schedule_transmission(15.0, 3)
	# Peppy barrel roll
	_schedule_transmission(20.0, 4)
...
func _on_start_menu_closed():
	#print("Main.gd -> _on_start_menu_closed() called!")
...
func _schedule_transmission(delay: float, message: int) -> void:
